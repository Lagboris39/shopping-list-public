...
### 4. iOS ホーム画面アプリの強制更新ボタン（優先度: 高）

**背景:**
iPhoneのホーム画面に追加したアプリは、ブラウザが画面の内容を手元に保存（キャッシュ）するため、アプリを更新しても古いバージョンが表示され続けることがある。

**解決策:**
設定メニュー（肉球ボタン）の中に「最新版に更新」ボタンを追加する。タップすると、ブラウザに保存された古い情報を無視して最新のデータをダウンロードし直す。

**実装箇所:**
- `src/App.jsx` の設定ドロワー内にボタンを追加
- タップ時に現在のURLに日時を付け加えて読み込み直す（例: `?v=1234567890`）
  ```js
  location.href = location.pathname + location.search.replace(/[?&]v=\d+/, '') +
    (location.search ? '&' : '?') + 'v=' + Date.now();
  ```
- ボタンは「🔄 最新版に更新」など分かりやすいラベルにする
