...
- [x] **一括追加モーダルがキーボードに隠れる問題** — `visualViewport` API でキーボード高さを検出し、モーダルの `bottom` を動的調整（`importModalRef` + `useEffect`）。
- [x] **星マーク付きアイテムを登録順に並べる** — `starredItems` のソートを `order_index` 基準に変更。
- [x] **共有テキストに星マーク（★）を付与** — `formatShareText()` で星付きアイテムを先頭に移動し `★ ` プレフィックスを付加。
- [x] **カード長押し中の押し込みアニメーション** — `.item-main-area:active { transform: scale(0.97); }` を `index.css` に追加。
- [x] **共有後の確認を中央ダイアログに変更** — `showShareConfirm` のモーダルをボトムシートから中央固定ダイアログ（spring アニメーション）に変更。
- [x] **星マーク機能 ON/OFF スイッチ** — `showStarFeature` state（`localStorage` 保存）を追加。設定ドロワー「カスタマイズ」にチェックボックストグルを追加。OFF 時は星ボタン・⭐グループを非表示。
- [x] **カテゴリ機能 ON/OFF スイッチ** — `showCategoryFeature` state（`localStorage` 保存）を追加。設定ドロワーにチェックボックストグルを追加。OFF 時は登録順フラット表示。
...
- **機能トグル**: `showStarFeature` / `showCategoryFeature` state。`localStorage` に `'showStarFeature'` / `'showCategoryFeature'` キーで保存。設定ドロワー「カスタマイズ」セクションのチェックボックスで切り替え。
- **一括追加キーボード対応**: `importModalRef` + `visualViewport` の `resize` イベントでモーダルの `bottom` を動的調整。`showImport` が `false` になると `bottom` をリセット。
