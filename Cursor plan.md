...
- [x] **並び替え吸い付き改善（Gemini plan 案3）** — 待機時間を300msに短縮。縦方向に8px以上動いたら即時ドラッグ開始（横スワイプと判別）。`latestEvent` はドラッグハンドル内のときのみ更新し、target 誤りによる「指についてこない」事象を回避。
...
### 機能追加
- [x] **カスタムカテゴリ** — 設定ドロワー「カスタマイズ」にカテゴリ名・絵文字・色を指定して追加するフォームを追加。`customCategories` を settings に保存し、リスト表示・カテゴリ選択・カテゴリ並び替えモーダルに統合。
...
モバイル実機で以下を確認。問題があれば閾値（8px→10px）や待機時間（300ms）を微調整。

- [ ] 縦8pxで即時ドラッグ開始した場合、スクロールとの競合がないか
- [ ] 横スワイプと縦ドラッグの境界（8px閾値）が適切か
- [ ] 隣のカードと重なったときのズレが改善されているか
...
設定ドロワー「カスタマイズ」内にカスタムカテゴリフォームを追加。カテゴリ名・絵文字・色を指定して追加。`getSetting`/`saveSetting` の `customCategories` キーで保存。`mergedCategoryNames`/`mergedCategoryColors`/`mergedCategoryIcons` で組み込みとマージ表示。
...
- **並び替え吸い付き（案3）**: ドラッグハンドル長押しで、縦8px以上かつ縦≥横の移動で即時 `dragControls.start(e)`。それ以外は300ms待機後に `dragControls.start(latestEvent)`。`latestEvent` は `handleRef.current?.contains(me.target)` のときのみ更新（target 誤り防止）。
